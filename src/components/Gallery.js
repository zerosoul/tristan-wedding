...
const weddings = Array.from(Array(29).keys()).map((v, idx) => (`w${idx + 1}`)).filter(p => !['w2', 'w6', 'w7', 'w13', 'w26', 'w21', 'w10'].includes(p));
const dailys = Array.from(Array(37).keys()).map((v, idx) => (`d${idx + 1}`)).filter(p => !['d7', 'd12', 'd15', 'd20', 'd21'].includes(p))
